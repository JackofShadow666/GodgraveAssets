...
if(_spiked2 && typeof GameTime!=='undefined'){
  ...
  const slowmoBtn = document.getElementById('mob-slowmo-btn');
  slowmoBtn?.addEventListener('touchstart', e => {
    e.preventDefault();
    enableAudioSystem();
    if(typeof cb === 'function' && !cb('slowmommb')) return;
  ...
  let lastSwordTap = 0;
  let swordDoubleTapAttack = false;
  ...
  function endSwordTouch(){
    ...
    if(swordDoubleTapAttack){
      swordDoubleTapAttack = false;
      mDown = false;
  ...
  zoneSword.addEventListener('touchstart', e => {
    ...
    if(now-lastSwordTap<300){
      swordDoubleTapAttack = true;
      mDown = true;
      if(typeof P !== 'undefined') P._shieldHeld = false;
