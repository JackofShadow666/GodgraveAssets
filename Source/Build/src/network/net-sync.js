...
var NET_SYNC = (function(){
  ...
  function buildDelta(p){
    ...
    d.wb=typeof wandBarrierHeld==='function' && wandBarrierHeld(p)?1:0;
    ...
    d.te=Math.round((p._tapeswordLinks||8)*100);
    d.tb=p._tapeswordGrowthBlocked?1:0;
    d.th=p._tapeswordHeld?1:0;
...
function onPvpReset(msg){
  ...
  function onState(msg){
    ...
    if(msg.t==='s'){
      ...
      if(msg.wb!=null) D._wandBarrierActive=msg.wb===1 && weaponKeyOf(D)==='wand';
      ...
      if(Number.isFinite(msg.te)) D._tapeswordLinks=$.M.clamp(msg.te/100,8,18);
      if(msg.tb!=null) D._tapeswordGrowthBlocked=msg.tb===1;
      if(msg.th!=null) D._tapeswordHeld=msg.th===1;
  ...
  function onProjectileContact(msg){
    ...
    const stun=msg && msg.stun==null ? 0 : msg.stun;
    if(!_active || !Number.isSafeInteger(msg.id) || msg.id<=_lastProjectileContact ||
       ![msg.damage,msg.stamina,msg.dx,msg.dy,msg.kx,msg.ky,disbalance,stun].every(Number.isFinite) ||
       msg.damage<0 || msg.damage>MAX_HP || (msg.stamina!==0 && msg.stamina!==30) ||
       disbalance<0 || disbalance>1.3 || stun<0 || stun>2 ||
