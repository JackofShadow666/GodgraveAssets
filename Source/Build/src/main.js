...
function markWandSwingCharge(ent){
  if(!ent || weaponKeyOf(ent)!=='wand' || !ent._wandGesture || !ent._wandGesture.held) return;
  ent._wandGesture.flickSeen=true;
  ent._wandGesture.mode='charge';
  ent._wandGesture.cancelled=false;
  ent._wandBarrierActive=false;
  if(typeof setWandBarrierLoop==='function') setWandBarrierLoop(ent,false);
}
function update(dt){
  if(typeof LocalPlayerControls!=='undefined') LocalPlayerControls.updateAutoSwordStyles();
  ...
  P.stunned = Math.max(0, (P._stunUntil || 0) - GameTime);
  ...
  const playerBotRangedHandled = !!(playerBotAI && P._playerBotRangedHandled);
  if(playerBotAI){
  ...
  if(playerBotRangedHandled){ mx=0; my=0; }
  if(mx||my){ const l=Math.hypot(mx,my); mx/=l; my/=l; }
  const pStunned = typeof isStunned === 'function' && isStunned(P);
  if(pStunned){ mx=0; my=0; mDown=false; P._shieldHeld=false; }
...
if(shieldDrainActive){
  ...
  const isTapesword = weaponKeyOf(P)==='tapesword';
  const inRageBuff = P.rageBuffEnd > GameTime;
  const lmbStaminaCost = P.stamMax * (sv('lmbcost') / 100) * weaponLmbStaminaMult(P);

  if(isTapesword){
    tapeswordInput(P,mDown,dt);
  } else if(isRangedWeapon(P) || weaponKeyOf(P)==='wand'){
    ...
    if(!playerBotRangedHandled) updateRangedWeaponFire(P, mDown);
    ...
    if(mDown && $.NOT(P, 'flail')){
      if(!P.lmbWasDown){
        if(typeof clearPostDodgeSlow==='function') clearPostDodgeSlow(P);
        P.lmbWasDown = true;
        ...
        if($.IS(P,'dagger') && GameTime >= (P._daggerPokeDodgeCD||0)){
          const cell = typeof CELL_PX === 'number' ? CELL_PX : 55;
          const impulse = (cell * 3) / 26;
          P._dvx = Math.cos(P.angle) * impulse;
          P._dvy = Math.sin(P.angle) * impulse;
          P._dodgeActiveUntil = Math.max(P._dodgeActiveUntil || 0, GameTime + 0.16);
          P._daggerPokeDodgeCD = GameTime + 0.35;
          if(typeof spawnDust==='function') for(let i=0;i<5;i++) spawnDust(P.x,P.y,-Math.cos(P.angle)*5,-Math.sin(P.angle)*5);
        }
...
const exhMult = getMod(P, 'moveSlow', 1);
  ...
  const wandLmbMoveMult = P._wandCharging ? 0.3 : 1;
  const maxV = 7 * pSpeedMult * sv('globalspd') * _shBaseMult * _shWrongMult * weaponMoveSpeedMult(P) * wandLmbMoveMult;
  ...
  if(actuallyMoving && !(GameTime < (P._postDodgeNoSwordBackUntil || 0))){
  ...
  const _pMoveLocked = GameTime < (P._moveLockUntil||0) || pStunned;
  ...
  const style = P.hasWeapon === false ? unarmedStyle : (isRanged ? getRangedStyle() : swordStyleForEntity(P));
  ...
  let dist = typeof gripDistForEntity==='function' ? gripDistForEntity(P, style) : style.dist;
...
const exhBodyMult = isExhausted(P) ? 0.3 : 1.0;
  ...
  if(meleePoseActive){
    ...
    let ex = style.ex;
    if(!isRanged && style.adaX !== false){
      const xBase = style.adaXb;
      const xPeak = style.adaXp;
      const t = Math.sin(ang) * Math.sin(ang);
      ex = xBase + (xPeak - xBase) * t;
    }

    let eyOffset = 0;
    if(style.adaY && P.hasWeapon !== false){
      const t = $.M.clamp(-Math.sin(ang), 0, 1);
      eyOffset -= t * (isRanged ? 60 : csv('adaY'));
    }
    if(style.adaD && P.hasWeapon !== false){
    ...
    if(style.ada12 && P.hasWeapon !== false){
      ...
      const longGrip12 = $.IS(P,'wand','staff','magicstaff','spear','halberd') ? 37 : (isRanged ? 25 : csv('ada12'));
      pivDownOffset = t12 * longGrip12;
    }
...
if (hasMod(P, 'weaponRecoil')) {
  ...
  if(_orbitDetected1 && !isExhausted(P) && $.NOT(P, 'flail')){
    ...
    $.S.play('whoosh');
    if($.IS(P,'tapesword') && (P._tapeswordLinks||0)>10) $.S.play('hammerSwing',0.35);
  }
  ...
  if(P.hasWeapon === false || isRangedWeapon(P) || $.IS(P, 'flail')){
    ...
    if(!P._swingBonusSuppressed && !isExhausted(P) && updateFlickDetect(P._realAngVel ?? P.vel, dt) && (P._swingBlockCD||0) < GameTime){
      if($.IS(P, 'flail') && P._flailExt < 0.97){
        ...
        markWandSwingCharge(P);
        $.FX.hit({x: rc.x + P.pvX, y: rc.y + P.pvY - 25, t:(window.I18N ? window.I18N.t('main.flick') : 'FLICK'), life:30, big:false, col:'#ffaa20'});
        if($.IS(P,'tapesword') && (P._tapeswordLinks||0)>10){ $.S.play('whoosh'); $.S.play('hammerSwing',0.35); }
        else $.S.play(isHeavySwingWeapon(P) ? 'hammerSwing' : 'whoosh');
      }
    }
    ...
    if(!P._swingBonusSuppressed && !isExhausted(P) && Math.abs(P.vel) > swingThreshold && (P._swingBlockCD||0) < GameTime){
      if($.IS(P, 'flail') && P._flailExt < 0.97){
        ...
        if(!P._swingFX){
          ...
          if($.IS(P,'tapesword') && (P._tapeswordLinks||0)>10){ $.S.play(P.rageBuffEnd>GameTime ? 'whooshRage' : 'whoosh'); $.S.play('hammerSwing',0.35); }
          else if(isHeavySwingWeapon(P)) $.S.play('hammerSwing');
...
function loop(ts){
  ...
  while(accumulator >= FIXED_DT && steps < maxSteps){
    ...
    if(D && D.hp > 0 && D._wandCharging) updateWandChargeParticles(dt, D);
...
if(dummyOn&&isUnbalanced(D)) drawUnbalancedStars(D); 
  ...
  if(doRender){
    ...
    if(!_pSwordBehind && P.hasWeapon !== false) drawSword(pivX, pivY, P.angle);
    if(!_pSwordBehind && P.hasWeapon === false) drawUnarmedHand(P, pivX, pivY, P.angle);
    if(typeof drawWandChargeOrb==='function'){
      drawWandChargeOrb(P);
      if(D && D.hp > 0) drawWandChargeOrb(D);
      if(dummyOn) for(const _b of ALL_BOTS) drawWandChargeOrb(_b);
    }
    ...
    if(typeof drawWandBarrier==='function'){
      drawWandBarrier(P);
      if(dummyOn) for(const _b of ALL_BOTS) drawWandBarrier(_b);
    }
