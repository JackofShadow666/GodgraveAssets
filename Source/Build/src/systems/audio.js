...
// не отвечает — раньше загрузчики ассетов могли зависнуть в
...
async function loadAudioDB() {
  ...
  for (const [type, folder] of Object.entries(SFX_FOLDERS)) {
    ...
    if (type === 'magicLoop')      matches = matches.filter(u => u.toLowerCase().includes('magicloop'));
...
const SFX_FIXED_URLS={
  ...
  magicLoop:[PROJECT_PATH_AUDIO+'Source/Sound/Magic/SFX_MagicLoop.wav']
};
...
function playControllableSound(sfxType, volume = 0.5, options = {}) {
  if (!audioEnabledFlag) return null;
  const arr = SFX_FIXED_URLS[sfxType] || SFX_DB[sfxType];
  if (!arr || arr.length === 0) {
    if (audioDBReady) console.warn('?? Нет звука для типа:', sfxType);
    return null;
  }
  ...
  const url = arr[idx];
  const audio = getPoolAudio(url);
  if(audio._fadeOutRAF) cancelAnimationFrame(audio._fadeOutRAF);
  audio.currentTime = 0;
