...
if(key === 'moveSlow' && ent && ent._postDodgeSlowStart && GameTime >= ent._postDodgeSlowStart && GameTime < (ent._postDodgeSlowUntil || 0)){
return Math.min(m && GameTime < m.until ? m.value : fallback, 0.1);
}
...
function applyPostDodgeSlow(ent){
if(!ent) return;
const start = Math.max(GameTime, ent._dodgeActiveUntil || GameTime);
ent._postDodgeSlowStart = start;
ent._postDodgeSlowUntil = start + 1;
ent._postDodgeNoSwordBackUntil = GameTime + 2;
}

function clearPostDodgeSlow(ent){
if(!ent || !ent._postDodgeSlowUntil) return;
ent._postDodgeSlowUntil = 0;
ent._postDodgeSlowStart = 0;
ent._postDodgeNoSwordBackUntil = 0;
}
...
if(ent){
ent.stunned = Math.max(0, (ent._stunUntil || 0) - GameTime);
if(ent._debuffType === 'stun' && (ent._debuffUntil || 0) <= GameTime){
...
isUsingStamina ||
isExhausted(ent) ||
(typeof wandBarrierHeld === 'function' && wandBarrierHeld(ent)) ||
(ent._exhaustedEndTime || 0) > GameTime
){
return;
}
...
function isStunned(ent){
return !!ent && (
(ent._debuffType === 'stun' && (ent._debuffUntil || 0) > GameTime) ||
(ent._stunUntil || 0) > GameTime
);
}
