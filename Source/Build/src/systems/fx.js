...
const LIGHTNING_HIT_TEXTURE_URL = '../VFX/T_Lightning.png';
let LIGHTNING_HIT_TEXTURE_IMG = null;

function lightningHitTexture(){
  if(LIGHTNING_HIT_TEXTURE_IMG) return LIGHTNING_HIT_TEXTURE_IMG;
  LIGHTNING_HIT_TEXTURE_IMG = typeof loadSpriteImage === 'function' ? loadSpriteImage(LIGHTNING_HIT_TEXTURE_URL) : null;
  return LIGHTNING_HIT_TEXTURE_IMG;
}

function lightningTextureFrame(img, seed){
  const frameW = 256, frameH = 64;
  const rows = Math.max(1, Math.floor((img && img.naturalHeight ? img.naturalHeight : 512) / frameH));
  const t = typeof GameTime === 'number' ? GameTime : 0;
  return {x:0,y:Math.floor((t * 24 + (seed || 0)) % rows) * frameH,w:frameW,h:frameH};
}
...
function spawnLightningHit(x, y, targetX, targetY, intensity, opts){
  ...
  LIGHTNING_HIT_FX.push({
    points: points,
    life: 1,
    maxLife: opts && opts.short ? (0.2 + intensity * 0.1) : (0.4 + intensity * 0.2), // живет дольше
    width: baseWidth,
    alpha: 1,
...
function drawLightningHitFX(){
  for(const bolt of LIGHTNING_HIT_FX){
    ...
    if(bolt.owner && typeof $ !== 'undefined' && $.POS && typeof $.POS.tip === 'function'){
      const tip = $.POS.tip(bolt.owner), target = bolt.target && bolt.target.hp > 0 && $.POS.body(bolt.target);
      const end = target || {
        x: tip.x + Math.cos(bolt.aimAngle || 0) * (bolt.range || 110),
        y: tip.y + Math.sin(bolt.aimAngle || 0) * (bolt.range || 110)
      };
      points[0] = {x:tip.x,y:tip.y};
      points[points.length - 1] = {x:end.x,y:end.y};
    }
    ctx.save();
    ctx.globalAlpha = a;

    const img = lightningHitTexture();
    const start = points[0], end = points[points.length - 1];
    if(img && img.complete && img.naturalWidth > 0 && start && end){
      const dx = end.x - start.x, dy = end.y - start.y;
      const len = Math.hypot(dx, dy);
      if(len > 0.01){
        const h = Math.max(32, Math.min(64, bolt.width * 5));
        ctx.save();
        ctx.globalAlpha = a;
        ...
        ctx.shadowBlur = 18;
        const f = lightningTextureFrame(img, bolt.seed);
