...
- `src/combat/tapesword.js`: 9/10/14-link tapesword growth, flexible segment geometry, fixed texture drawing, LMB cast/rage economy, forced 1-second LMB fold, standard LMB pose cleanup, block growth lock, repeated full-chain hits, bot hold timing, and local ownership of online damage.
- `src/combat/tapesword.test.cjs`: isolated timing, stamina, rage and growth-lock checks; run `node src/combat/tapesword.test.cjs`.
...
- `src/combat/ranged.js`: projectiles, wand/crossbow fire, ranged bot behavior; wand LMB gesture branches (quick electric strike, static semicircle barrier, same-direction 300-degree shot charge); unified projectile block cost (30 on defender), wand impact impulse/disarm, final damage multiplier after caps, arrow charge-time metadata for bot dodge chance, offscreen ranged-threat screen-edge markers.
...
- `src/input/player-controls.js`: local multiplayer slots, synthetic per-player input, local PvP control routing, and per-local-player automatic Classic/Fencer stance selection by nearest attackable enemy.
...
- `src/network/net-relay.js`: WebSocket relay auto-match transport; patches `NET_CORE`/`$.NET` so existing PvP sync can run through a relay without manual IDs.
- `src/network/relay-server.cjs`: local dependency-free Node WebSocket relay for testing auto-match before deploying a hosted relay.
...
- Tapesword: definition/fixed hand+ring texture loading -> `src/combat/weapons.js`; growth, flexible links, contacts and stamina -> `src/combat/tapesword.js`; ordinary collision exclusion -> `src/combat/combat.js`; render -> `src/arena/arena.js`; online length/hold/block state -> `src/network/net-sync.js`.
...
- Post-dodge slow: `src/input/player-controls.js` schedules `moveSlow=0.1` for 1 second after the dodge impulse ends via `src/systems/buff.js`; ordinary dodge distance is 1.5 cells; sword-back speed boost is disabled for 2 seconds after dodge; `src/main.js` clears the slow on attack start and `src/combat/combat.js` clears it on block.
- Dagger poke/passive: `src/main.js` sets dagger grip distance to 20 and starts a short 3-cell forward impulse on LMB poke; `src/ai/ai.js` mirrors the shorter dagger poke impulse for bots; `src/combat/ranged.js` and `src/combat/combat.js` apply dagger passive `moveSlow=0.5` for 3 seconds on damage or weapon clash.
...
- Wand LMB gesture: `src/combat/ranged.js` owns the 0.2-second quick lightning release, 0.3-second flick-to-charge or no-flick shield split, 50-rage quick lightning/recoil, free-rotating 3-cell diameter barrier, charged shot release, visual-tip projectile spawn, collecting-to-tip charge particles, and wand bot shield/lightning/flick-start shot actions; `src/combat/weapons.js` keeps wand out of `isRangedWeapon` so bots treat a wand user as melee while `src/ai/ai.js` routes wand melee like axe swing-only instead of thrust/probe, and `src/main.js` still routes local wand LMB to ranged handling and cancels player movement while stunned; `src/main.js`/`src/arena/arena.js` set 12-o'clock pivot height to 37 for wand/staff/magicstaff/spear/halberd; `src/systems/buff.js` suppresses stamina regeneration while the barrier is active; melee barrier contact and bot-flank trigger are in `src/combat/combat.js`, the behind-target route is in `src/ai/ai.js`, thrown reflection in `src/combat/weapons.js`, rotating barrier texture drawing in `src/arena/arena.js`, quick-lightning texture drawing in `src/systems/fx.js`, and online barrier/stun state in `src/network/net-sync.js`.
