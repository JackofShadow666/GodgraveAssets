...
$.E = $.E || {
  ...
  chargeShake(ent){
    return !!(ent && (($.K.is(ent, 'wand') && ent._wandChargeFXActive) || ($.K.is(ent, 'magicstaff') && ent._magicCharging)));
  },
  shieldOff(ent){
...
$.A = $.A || {
  meleeHold(ent, held){
    const key = ent && typeof weaponKeyOf==='function' ? weaponKeyOf(ent) : null;
    if(key==='tapesword') return !!(ent._tapeswordHeld || ent._tapeswordPress);
    if(key==='wand') return !!held && (ent.rage||0) >= 50;
