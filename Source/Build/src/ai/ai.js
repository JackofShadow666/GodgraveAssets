...
const NO_THRUST_WEAPON_KEYS = ['axe', 'hammer', 'flail', 'wand', 'tapesword'];
const SWING_ONLY_WEAPON_KEYS = ['axe', 'hammer', 'flail', 'wand'];
const SWING_BURST_WEAPON_KEYS = ['sword', 'tapesword', 'longsword', 'greatsword', 'staff', 'magicstaff', 'wand'];
...
function aiStartHeavyAttack(ai, bot, distToPlayer, cscl, immediateRelease, angToPlayer){
  ...
  if(weaponKeyOf(bot)==='tapesword'){
    ai._tapeswordAttackCount=(ai._tapeswordAttackCount||0)+1;
    ai._tapeswordUseLmbAttack=(ai._tapeswordAttackCount%2)===0;
    if(ai._tapeswordUseLmbAttack){
      ai._tapeswordLmbSpinActive=true;
      ai._tapeswordLmbSpinStart=GameTime;
      ai._tapeswordLmbSpinBase=Number.isFinite(angToPlayer)?angToPlayer:bot.angle;
      ai._tapeswordLmbSpinSide=ai._heavySwingSide||1;
      ai._tapeswordLmbHoldUntil=GameTime+1.0;
    }
  } else {
    ai._tapeswordUseLmbAttack=false;
  }
  ...
  ai._heavySwingSoundPlayed = false;
  ai._heavyStaminaSpent = false;
  const dur = immediateRelease ? aiHeavyReleaseDuration(ai) : (weaponKeyOf(bot)==='tapesword' && ai._tapeswordUseLmbAttack ? 0.08 : rf(0.22, 0.06));
...
function aiUpdateHeavyAttack(ai, bot, k, bBodyC, pBodyC, angToPlayer, distToPlayer, cscl){
  ...
  const tapeReach=weaponKeyOf(bot)==='tapesword';
  const tapeApproach=tapeReach ? 170 * cscl : 75 * cscl;
  if(ai._heavyAttackPhase === 'windup' && distToPlayer > (tapeReach ? 235 * cscl : 125 * cscl)) aiMoveToward(k, bBodyC, pBodyC, tapeApproach, cscl);
  ...
  if(weaponKeyOf(bot)==='tapesword' && ai._tapeswordLmbSpinActive){
    const spinStart=Number.isFinite(ai._tapeswordLmbSpinStart)?ai._tapeswordLmbSpinStart:GameTime;
    const spinBase=Number.isFinite(ai._tapeswordLmbSpinBase)?ai._tapeswordLmbSpinBase:angToPlayer;
    const spinT=$.M.clamp((GameTime-spinStart)/0.8,0,1);
    const spinAng=spinBase+(ai._tapeswordLmbSpinSide||1)*(200*Math.PI/180)*spinT;
    const pivot=$.POS.pivot(bot);
    aiPointMouse(bBodyC,pivot.x+Math.cos(spinAng)*190*cscl,pivot.y+Math.sin(spinAng)*190*cscl,false,ai);
    ai._fakeMDown=true;
    k.w=k.a=k.s=k.d=false;
    const holdUntil=Number.isFinite(ai._tapeswordLmbHoldUntil)?ai._tapeswordLmbHoldUntil:spinStart+1.0;
    if(spinT>=1 && GameTime>=holdUntil){
      ai._tapeswordLmbSpinActive=false;
      ai._fakeMDown=false;
      ai._tapeswordLmbHoldUntil=0;
      bot._tapeswordForceRelease=false;
      bot._tapeswordAIWasDown=false;
    }
    return true;
  }
  ai._fakeMDown = ai._heavyAttackPhase === 'release' && weaponKeyOf(bot)!=='tapesword';
  if(ai._fakeMDown && !ai._heavyStaminaSpent && !$.IS(bot,'flail','tapesword')){
...
function aiTapeswordThreat(target, bot, distToPlayer, cscl){
  if(!target || !bot || weaponKeyOf(target)!=='tapesword') return {active:false, block:false, evade:false};
  const targetAi=target._aiState||{};
  const extending=!!(target._tapeswordHeld || target.lmbWasDown || targetAi._fakeMDown);
  if(!extending) return {active:false, block:false, evade:false};
  if(distToPlayer >= 260*cscl) return {active:false, block:false, evade:false};
  const key=weaponKeyOf(bot);
  const hasShield=bot.shield>0;
  const poorBlock=!hasShield && $.ISK(key,'bow','crossbow','dagger','wand');
  return {active:true, block:!poorBlock, evade:poorBlock};
}
...
function estimateThrowRange(def){
 ...
 function updateAI(dt, bot){
  ...
  ai._shieldHeld = false;
  if(targetPlayer === P){
    window._lastPlayerTargetingBot = bot;
    ai._lastTargetedPlayerAt = GameTime;
  }
  if(!ai.enabled){
...
if(ai._contactCD > 0 && ai.phase === 'attack' && ai._contactCD <= GameTime){
  ...
  if(!ai._pokeDodgeActive && GameTime >= ai._pokeDodgeTimer){
    ...
    if(_canPokeDodge){
      if(Math.random() < 0.5 && aiCanUseShield(bot, ai)){
        ...
        const pokeImpulse = $.IS(bot,'dagger') ? ((typeof CELL_PX==='number'?CELL_PX:55)*3/26) : 5.6;
        bot._dvx = Math.cos(angToPlayer)*pokeImpulse;
        bot._dvy = Math.sin(angToPlayer)*pokeImpulse;
  ...
  if(bot.exhausted > 0 || (typeof isStunned==='function' && isStunned(bot)) || hasMod(bot, 'weaponRecoil')){
    k.w=k.a=k.s=k.d=false;
    ai._fakeMDown=false;
    aiPointMouse(bBodyC, pBodyC.x, pBodyC.y, false, ai);
    return;
  ...
  const barrierTarget=ai._wandBarrierFlankTarget;
  if(barrierTarget && barrierTarget.hp>0 && (GameTime<(ai._wandBarrierFlankUntil||0) || (typeof wandBarrierHeld==='function' && wandBarrierHeld(barrierTarget)))){
    const tc=$.POS.body(barrierTarget);
    const behind={x:tc.x-Math.cos(barrierTarget.angle||0)*105*cscl,y:tc.y-Math.sin(barrierTarget.angle||0)*105*cscl};
    ai._fakeMDown=false;
    aiMoveToward(k,bBodyC,behind,35*cscl,cscl);
    aiPointMouse(bBodyC,tc.x,tc.y,false,ai);
    if(typeof wandBarrierHeld==='function' && wandBarrierHeld(barrierTarget)) ai._wandBarrierFlankUntil=GameTime+0.5;
    else if(Math.hypot(behind.x-bBodyC.x,behind.y-bBodyC.y)<40*cscl) ai._wandBarrierFlankTarget=null;
    return;
  }
  ai._wandBarrierFlankTarget=null;

  const tapeThreat = aiTapeswordThreat(targetPlayer, bot, distToPlayer, cscl);
  if(tapeThreat.active){
    if(tapeThreat.evade){
      aiMoveAway(k, bBodyC, pBodyC, 280*cscl, cscl);
      if((ai._botDodgeCooldown||0)<=0 && bot.stamina>=30) aiStartHeavyEvasiveDodge(ai, bot, angToPlayer);
    } else {
      k.w=k.a=k.s=k.d=false;
      const guardAng=angToPlayer + Math.PI/2 + ((targetPlayer.vel||0)>0 ? -0.3 : 0.3);
      const dpivGuard=$.POS.pivot(bot);
      aiPointMouse(bBodyC, dpivGuard.x+Math.cos(guardAng)*140*cscl, dpivGuard.y+Math.sin(guardAng)*140*cscl, false, ai);
  ...
  if(ai.tactic === 'COMBAT_HARASS'){
    ...
    if(hp === 'approach'){
      ...
      const harassStrikeDist = weaponKeyOf(bot)==='tapesword' && bot.stamina>30 ? 210*cscl : 90*cscl;
      if(distToPlayer < harassStrikeDist){
      ...
      const tapeStrike = weaponKeyOf(bot)==='tapesword';
      if(aiUsesSwingBurst(ai, bot)){
        aiStartHeavyAttack(ai, bot, distToPlayer, cscl, distToPlayer < (tapeStrike ? 210 : 80) * cscl, angToPlayer);
        if(aiUpdateHeavyAttack(ai, bot, k, bBodyC, pBodyC, angToPlayer, distToPlayer, cscl)) return;
      }
      if(tapeStrike && distToPlayer > 110*cscl){
        aiMoveToward(k, bBodyC, pBodyC, 105*cscl, cscl);
        aiPointMouse(bBodyC, pBodyC.x, pBodyC.y, false, ai);
        ai._fakeMDown = false;
      } else {
        k.w=k.a=k.s=k.d=false;
      ...
      ai._fakeMDown = weaponKeyOf(bot)==='tapesword' && bot.stamina>30;
...
if(!ai._feintActive){
  if(ai._duelPull){
    ...
    const attackDist = weaponKeyOf(bot)==='tapesword' ? 165 * cscl : (isPlayerRanged ? 50 * cscl : 80 * cscl);
    ...
    if(!ai._spinActive){
      ...
      if(aiUsesSwingBurst(ai, bot)){
        const tape=weaponKeyOf(bot)==='tapesword';
        const startDist=(tape?255:135)*cscl;
        const immediateDist=(tape?210:85)*cscl;
        if(distToPlayer < startDist && GameTime >= (ai._heavyAttackCD || 0)){
          aiStartHeavyAttack(ai, bot, distToPlayer, cscl, distToPlayer < immediateDist, angToPlayer);
  if(aiUpdateHeavyAttack(ai, bot, k, bBodyC, pBodyC, angToPlayer, distToPlayer, cscl)) return;
        } else {
          const guardAim = aiHeavyAimPoint(bot, pBodyC, angToPlayer, cscl, ai._heavySwingSide || 1, 'recover', 0, angToPlayer);
          aiPointMouse(bBodyC, guardAim.x, guardAim.y, false, ai);
  ...
  // ── ПРОКРУТ МЕЧА ──────────────────────────────────
  if(ai._spinActive){
    if(ai._spinLmbRollEnd !== ai._spinEndTime){
      ai._spinLmbRollEnd = ai._spinEndTime;
      ai._spinLmbUntil = weaponKeyOf(bot)==='tapesword' && bot.stamina>20 && Math.random()<0.8 ? ai._spinEndTime : 0;
    }
    if(GameTime >= ai._spinEndTime){
      ai._spinActive = false;
      ai._spinLmbRollEnd = 0;
      ai._spinLmbUntil = 0;
      if(weaponKeyOf(bot)==='tapesword') ai._fakeMDown = false;
      ai._mirrorCooldown = GameTime + 1.0;
      ai._smoothMX = pBodyC.x; ai._smoothMY = pBodyC.y;
      ai._fakeMX = pBodyC.x;   ai._fakeMY = pBodyC.y;
    } else {
      if(weaponKeyOf(bot)==='tapesword' && GameTime < (ai._spinLmbUntil||0)) ai._fakeMDown = true;
      const spinSpeedMult = weaponKeyOf(bot)==='tapesword' ? 1.5 : 1;
      ai._spinAng -= ai._spinSpeed * spinSpeedMult * dt;
  ...
  const finalTapeThreat = aiTapeswordThreat(targetPlayer, bot, distToPlayer, cscl);
  const wantsDefensiveShield = finalTapeThreat.active || ai.phase === 'retreat' || ai.phase === 'breather' || ai._duelistBlocking || (ai.tactic === 'COMBAT_HARASS' && ai._harassPhase === 'orbit');
...
function updatePlayerBotAssistAI(dt){
  if(!P || P.hp <= 0 || !P._playerBotMode){
    if(P) P._playerBotRangedHandled = false;
  ...
  if(!target){
    P._playerBotRangedHandled = false;
    ai._fakeMDown=false;
  ...
  P._playerBotRangedHandled = false;
  const pKey = weaponKeyOf(P);
  if($.ISK(pKey,'bow','crossbow') && typeof updateCrossbowBotAI==='function'){
    updateCrossbowBotAI(dt, P);
    P._playerBotRangedHandled = true;
  } else if($.ISK(pKey,'wand','magicstaff') && typeof updateWandBotAI==='function'){
    const handled = updateWandBotAI(dt, P) !== false;
    P._playerBotRangedHandled = handled;
    if(!handled) updateAI(dt, P);
  } else {
    updateAI(dt, P);
  }
  P._aiTargetOverride = null;
