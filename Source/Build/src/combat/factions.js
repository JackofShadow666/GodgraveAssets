(function(){
  ...
  function nearestEnemy(ent){
    if(!ent) return null;
    const candidates = players().concat(typeof ALL_BOTS === 'undefined' ? [] : ALL_BOTS);
    const ec = typeof $ !== 'undefined' && $.POS ? $.POS.body(ent) : ent;
    let nearest = null;
    let nearestD = Infinity;
    for(const candidate of candidates){
      if(typeof dummyOn!=='undefined' && !dummyOn && !isPlayer(candidate)) continue;
      if(!candidate || candidate === ent || candidate.hp <= 0 || candidate._defeated ||
         candidate._awaitingReveal || !canFight(ent, candidate)) continue;
      const cc = typeof $ !== 'undefined' && $.POS ? $.POS.body(candidate) : candidate;
      const distance = Math.hypot(cc.x - ec.x, cc.y - ec.y);
      if(distance < nearestD){ nearest = candidate; nearestD = distance; }
    }
    return nearest;
  }
