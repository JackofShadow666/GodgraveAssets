...
function swordHit(entA, entB){
  ...
  if(typeof clearPostDodgeSlow==='function') clearPostDodgeSlow(defender);
...
function applyDaggerPassiveSlow(attacker, defender){
  if(!attacker || !defender || attacker.hasWeapon===false || defender.hp<=0) return;
  if(weaponKeyOf(attacker)!=='dagger') return;
...
function checkSwordCollision(entA, entB, dt){
  const tapeA=weaponKeyOf(entA)==='tapesword', tapeB=weaponKeyOf(entB)==='tapesword';
  if(tapeA || tapeB){
    for(const [attacker,defender] of [[entA,entB],[entB,entA]]){
      if(weaponKeyOf(attacker)==='tapesword') continue;
...
const bodySwB = weaponReach(entB) * sv('swlen') * (isBot(entB)?sv('botswordscale'):1) * BODY_HIT_RATIO;
  ...
  if(colA !== 'none' && colB !== 'none' && !isWeaponDisabled(entA) && !isWeaponDisabled(entB)){
    ...
    if(entA._bladeCD <= GameTime){
      ...
      applyDaggerPassiveSlow(entA, entB);
      applyDaggerPassiveSlow(entB, entA);
      swordHit(entA, entB);
...
function applyInterruptingKnockback(defender, dirX, dirY, power, unbalanceDur){
  if(!defender) return;
  defender.vx = 0;
  defender.vy = 0;
  defender._dvx = dirX * power;
  defender._dvy = dirY * power;
  defender._dodgeActiveUntil = Math.min(defender._dodgeActiveUntil || 0, GameTime);
  defender._moveLockUntil = Math.max(defender._moveLockUntil || 0, GameTime + 0.25);
  defender.unbalanced = Math.max(defender.unbalanced || 0, unbalanceDur || 0);
}
...
function checkBladeVsBody(attacker, defender, pivX, pivY, tipX2, tipY2) {
  ...
  if(typeof wandBarrierSegmentIntersects==='function' && wandBarrierSegmentIntersects(defender,pivX,pivY,tipX2,tipY2,10)){
    const mx=(pivX+tipX2)/2,my=(pivY+tipY2)/2;
    applyShieldBlockFX(mx,my,attacker,defender,{hitstopMag:3,waveDuration:22,unbalanceAmt:1.2,textLife:35});
    const dx=tipX2-pivX,dy=tipY2-pivY,len=Math.hypot(dx,dy)||1;
    attacker.vx-=dx/len*2; attacker.vy-=dy/len*2;
    if(attacker.hasWeapon!==false && Math.random()<0.3 && typeof disarmEntity==='function') disarmEntity(attacker,-dx/len*3,-dy/len*3);
    if(attacker._aiState){
      attacker._aiState._wandBarrierFlankTarget=defender;
      attacker._aiState._wandBarrierFlankUntil=GameTime+4;
    }
    return;
  }
  const key = weaponKeyOf(attacker);
        ...
        if (tipX_adj >= _shL && tipX_adj <= _shR && tipY_adj >= _shT && tipY_adj <= _shB) {
          ...
          if ((defender._shieldAlpha || 1) > 0.4) {
            ...
            if (weaponHasFlag(attacker, 'knockback_hammer')) {
              applyInterruptingKnockback(defender, -_bvx / _bl, -_bvy / _bl, 18, 0.8);
            }
            if (weaponHasFlag(attacker, 'knockback_staff')) {
              applyInterruptingKnockback(defender, -_bvx / _bl, -_bvy / _bl, 8, 0.4);
            }
  ...
  if (defender._hitCD < GameTime) {
    ...
    if (finalCondition) {
      ...
      if (weaponHasFlag(attacker, 'knockback_hammer')) {
        ...
        applyInterruptingKnockback(defender, _kkX / _kkL, _kkY / _kkL, 18 * defenderShieldKbMult, 0);
      ...
      if (weaponHasFlag(attacker, 'knockback_staff')) {
        ...
        const _kkY = bC.y - $.POS.body(attacker).y;
        const _kkL = Math.hypot(_kkX, _kkY) || 1;
        applyInterruptingKnockback(defender, _kkX / _kkL, _kkY / _kkL, 10 * defenderShieldKbMult, 0);
        if(!isUnbalanced(defender)) applyDisbalance(defender, attacker);
      }
...
function applyDebuff(ent, type, duration, intensity) {
    ...
    if (type === 'stun') {
        ent.stunned = duration;
        ent._stunUntil = GameTime + duration;
    }
...
function swordStyleForEntity(ent){
  if(ent && ent._autoSwordStyle){
    const st=ent._autoSwordStyle, scale=sv('cscl');
    return {...st,dist:st.dist*scale,ex:st.ex*scale,ey:st.ey*scale,adaXb:st.adaXb*scale,adaXp:st.adaXp*scale};
  }
  if(ent && ent !== P && ent._aiState && ent._aiState._styleVals) return ent._aiState._styleVals;
  return {
    name:SWORD_STYLES[window.SWORD_STYLE_IDX]?.name,
    dist:csv('dist'), ex:csv('ex'), ey:csv('ey'), blk:sv('blk'),
    adaXb:csv('adaXb'), adaXp:csv('adaXp'), adaY:cb('adaY'),
    adaD:cb('adaD'), ada12:cb('ada12'), adaX:cb('adaX')
  };
}

function gripDistForEntity(ent, style){
  if($.IS(ent,'dagger')) return 20;
  if($.IS(ent,'wand') && style && style.name==='Classic') return 31 * sv('cscl');
  return style ? style.dist : 0;
}

function disableAutoSwordStyle(){
  const checkbox=document.getElementById('cb-autostance');
  if(checkbox && checkbox.checked){
    checkbox.checked=false;
    checkbox.dispatchEvent(new Event('change',{bubbles:true}));
  }
  if(typeof LocalPlayerControls!=='undefined') LocalPlayerControls.clearAutoSwordStyles();
}
...
window.toggleSwordStyle = function(){
  const automatic=P && P._autoSwordStyle;
  disableAutoSwordStyle();
  const currentIdx=automatic ? Math.max(0,SWORD_STYLES.indexOf(automatic)) : window.SWORD_STYLE_IDX;
  window.applySwordStyle((currentIdx+1)%SWORD_STYLES.length);
};

window.toggleSwordStyleForEntity = function(ent){
  if(!ent || ent===P){ window.toggleSwordStyle(); return; }
  const current=ent._autoSwordStyle || (ent._aiState && ent._aiState._styleVals);
  disableAutoSwordStyle();
  const currentIdx=Math.max(0,SWORD_STYLES.indexOf(current));
  const nextIdx=(currentIdx+1)%SWORD_STYLES.length;
  if(ent._aiState) ent._aiState._styleVals=SWORD_STYLES[nextIdx];
  if(typeof $!=='undefined' && $.FX){
    const st=SWORD_STYLES[nextIdx];
    $.FX.hit({x:ent.x,y:ent.y-45,t:(window.I18N?window.I18N.t('combat.style',{name:st.name}):('STYLE: '+st.name)),life:55,big:false,col:'#9ad0f0'});
  }
};
...
// Applies the defender-side mechanics of a melee clash when a projectile hits a blade.
// The shooter is deliberately unaffected: only the fighter holding the struck weapon recoils.
function applyProjectileBladeClash(defender, hitX, hitY, projectileVX, projectileVY){
  if(!defender) return;
  const reach = typeof weaponReach === 'function' ? weaponReach(defender) * sv('swlen') : SWORD_LEN;
  const recoilMult = $.M.clamp(SWORD_LEN / Math.max(SWORD_LEN, reach), 0.45, 1);
  const impactSpeed = Math.hypot(projectileVX || 0, projectileVY || 0);
  const deflectDeg = Math.min(getDynamicDeflectMax(), sv('deflectMin') + impactSpeed * 20);
  const deflectRad = deflectDeg * recoilMult * Math.PI / 180;
  const piv = $.POS.pivot(defender);
  const relX = hitX - piv.x, relY = hitY - piv.y;
  const cross = Math.cos(defender.angle) * relY - Math.sin(defender.angle) * relX;
  const deflectDir = cross >= 0 ? -1 : 1;
  defender.angle += deflectDir * deflectRad;
  defender.vel = $.M.clamp(deflectDir * 1.5 * recoilMult, -8, 8);
  const push = Math.min(sv('bladeKB'), 25);
  if(push > 0 && impactSpeed > 0){
    defender._dvx = (defender._dvx || 0) + projectileVX / impactSpeed * push;
    defender._dvy = (defender._dvy || 0) + projectileVY / impactSpeed * push;
  }
  defender._moveLockUntil = Math.max(defender._moveLockUntil || 0, GameTime + 0.35);
  const slowDuration = sv('blockSlowDur');
  if(slowDuration > 0) defender._blockSlow = GameTime + slowDuration;
  defender._swingBlockCD = GameTime + 0.25;
}
...
function checkShieldVsBlade(attacker, defender, bx1,by1, tx1,ty1){
  if(typeof wandBarrierContains==='function' &&
     (wandBarrierContains(defender,tx1,ty1,8) || wandBarrierContains(defender,bx1,by1,8) ||
      (typeof wandBarrierSegmentIntersects==='function' && wandBarrierSegmentIntersects(defender,bx1,by1,tx1,ty1,8)))){
    const mx=(bx1+tx1)/2,my=(by1+ty1)/2;
    applyShieldBlockFX(mx,my,attacker,defender,{hitstopMag:3,waveDuration:22,unbalanceAmt:1.2,textLife:35});
    const dx=tx1-bx1,dy=ty1-by1,len=Math.hypot(dx,dy)||1;
    attacker.vx-=dx/len*2; attacker.vy-=dy/len*2;
    if(attacker.hasWeapon!==false && Math.random()<0.3 && typeof disarmEntity==='function') disarmEntity(attacker,-dx/len*3,-dy/len*3);
    if(attacker._aiState){
      attacker._aiState._wandBarrierFlankTarget=defender;
