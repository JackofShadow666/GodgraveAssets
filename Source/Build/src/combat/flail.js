...
function drawFlailSprite(ctx2, ent, length, glowColor, glowBlur){
    ...
    const headLen = FLAIL_HEAD_LEN * flailHeadScaleFor(ent);
    // Длина цепи = общая длина - навершие
    const chainLen = Math.max(0, length - headLen);
    ...
    if(headImg && headImg.complete && headImg.naturalWidth > 0){
        const hw = headLen * spriteAspectFor(headImg);
        const headLag = lagAmount * 0.9; // сильнее провисание на конце
        const headOverlap = Math.max(2, headLen * 0.2);
        ctx2.drawImage(headImg, -hw/2 + headLag, cursor + headOverlap, hw, -headLen);
    }
...
function flailHeadScaleFor(ent){
  return 1.3 * (0.5 + 0.5 * $.M.clamp(ent._flailExt || 0, 0, 1));
}
...
function drawFlailChain(c,ent,glow,blur){
    ...
    const last=nodes[nodes.length-1],img=ent._weaponImg,h=FLAIL_HEAD_LEN*scale*flailHeadScaleFor(ent);
