...
// ── Оружие ───────────────────────────────────────────────────────────────
// Список видов оружия. Баланс оружия хранится только здесь.
const WEAPON_TYPES = [
  { key:'sword',       name:'Меч',              category:'sword',      throwSpeed: 10,  scale: 1.0, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.24, spinMax: 0.48, weight: 1.0, staminaMult: 1.0,  cutMult: 1.0,  pierceMult: 1.0,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'rapier',      name:'Рапира',           category:'rapier',     throwSpeed: 6,   scale: 1.1, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.24, spinMax: 0.48, weight: 0.5, staminaMult: 1.0,  cutMult: 0.3,  pierceMult: 2.5,  collision:'full', flags:'',                            lmbStaminaMult: 0.8 },

  // В старой таблице tapesword отсутствует — числа оставлены из новой версии.
  { key:'tapesword',   name:'Ленточный меч',    category:'tapesword',  throwSpeed: 6,   scale: 1.0, dmgBase: 6,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.24, spinMax: 0.48, weight: 1.2, staminaMult: 1.0,  cutMult: 0.8,  pierceMult: 0.8,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'dagger',      name:'Кинжал',           category:'knife',      throwSpeed: 14,  scale: 0.5, dmgBase: 12, dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.24, spinMax: 0.48, weight: 0.4, staminaMult: 0.01, cutMult: 1.0,  pierceMult: 3.5,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'spear',       name:'Копьё',            category:'spear',      throwSpeed: 16,  scale: 2.0, dmgBase: 14, dmgPerSpeed: 6, maxDmgPercent: 0.30, spinMin: 0,    spinMax: 0,    weight: 0.8, staminaMult: 0.3,  cutMult: 0.1,  pierceMult: 3.0,  collision:'tip',  flags:'',                            lmbStaminaMult: 1.0 },

  { key:'halberd',     name:'Алебарда',         category:'halberd',    throwSpeed: 8,      scale: 1.7, dmgBase: 4,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.2,    spinMax: 0,    weight: 1.4, staminaMult: 1.4,  cutMult: 1.2,  pierceMult: 1.5,  collision:'tip',  flags:'',                            lmbStaminaMult: 1.0 },

  { key:'axe',         name:'Топор',            category:'axe',        throwSpeed: 14,     scale: 0.9, dmgBase: 12, dmgPerSpeed: 6, maxDmgPercent: 0.30, spinMin: 0.24, spinMax: 0.24, weight: 1.2, staminaMult: 1.3,  cutMult: 1.7,  pierceMult: 0.2,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'longsword',   name:'Длинный меч',      category:'sword',      throwSpeed: 8, scale: 1.3, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.1,  spinMax: 0.24, weight: 1.2, staminaMult: 1.0,  cutMult: 1.1,  pierceMult: 1.2,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },
  //{ key:'mace',   name:'Булава',      category:'hammer',      throwSpeed: 6, scale: 1.3, dmgBase: 4,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.1,  spinMax: 0.24, weight: 1.0, staminaMult: 1.2,  cutMult: 1.5,  pierceMult: 0.2,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'greatsword',  name:'Огромный меч',     category:'sword',      throwSpeed: 3.5,  scale: 1.7, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.05, spinMax: 0.1,  weight: 1.4, staminaMult: 1.9,  cutMult: 1.5,  pierceMult: 1.2,  collision:'full', flags:'',                            lmbStaminaMult: 1.0 },

  { key:'hammer',      name:'Молот',            category:'hammer',     throwSpeed: 12, scale: 1.0, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.1,  spinMax: 0.2,  weight: 1.2, staminaMult: 1.8,  cutMult: 2.5,  pierceMult: 0.1,  collision:'full', flags:'disarm,knockback_hammer',       lmbStaminaMult: 1.0 },

  { key:'staff',       name:'Посох',            category:'staff',      throwSpeed: 7, scale: 1.8, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.1,  spinMax: 0.2,  weight: 0.8, staminaMult: 0.5,  cutMult: 0.4,  pierceMult: 0.4,  collision:'full', flags:'knockback_staff',              lmbStaminaMult: 1.0 },

  { key:'magicstaff',  name:'Магический посох', category:'magicstaff', throwSpeed: 7, scale: 1.8, dmgBase: 8,  dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.1,  spinMax: 0.2,  weight: 0.8, staminaMult: 1.5,  cutMult: 0.5,  pierceMult: 0.4,  collision:'full', flags:'ranged_magicstaff', chargeTime: 2.0, lmbStaminaMult: 1.0 },

  { key:'flail', damageMult:0.5, name:'Цеп', category:'flail', throwSpeed: 7, scale: 0.7,
    dmgBase: 8, dmgPerSpeed: 6, maxDmgPercent: 0.20, spinMin: 0.24, spinMax: 0.24,
    weight: 0.3, staminaMult: 1.5, cutMult: 1.5, pierceMult: 0.1,
    collision:'tip', flags:'disarm,knockback', lmbStaminaMult: 1.0 },

  { key:'wand',        name:'Жезл',             category:'wand',       throwSpeed: 7,     scale: 1.0, dmgBase: 10, dmgPerSpeed: 0, maxDmgPercent: 0.25, spinMin: 0.1,  spinMax: 0.1,  weight: 1.2, staminaMult: 1.5,  cutMult: 0.5,  pierceMult: 0.3,  collision:'full', flags:'ranged_wand',       chargeTime: 0.5, lmbStaminaMult: 1.0 },

  { key:'bow',         name:'Лук',              category:'bow',        throwSpeed: 4,     scale: 0.3, dmgBase: 2,  dmgPerSpeed: 0, maxDmgPercent: 0.20, spinMin: 0.1,    spinMax: 0,    weight: 0.8, staminaMult: 0.2,  cutMult: 0.01, pierceMult: 0.01, collision:'none', flags:'ranged_bow',        chargeTime: 1.0, lmbStaminaMult: 1.0 },

  { key:'crossbow',    name:'Арбалет',          category:'crossbow',   throwSpeed: 4,      scale: 0.4, dmgBase: 5,  dmgPerSpeed: 0, maxDmgPercent: 0.30, spinMin: 0.1,    spinMax: 0,    weight: 1.0, staminaMult: 0.1,  cutMult: 0.01, pierceMult: 0.01, collision:'none', flags:'ranged_crossbow',                     lmbStaminaMult: 1.0 },
];
...
function bowVisualPixelLen(){
  const cell = (typeof CELL_PX === 'number' && CELL_PX > 0) ? CELL_PX : 55;
  return cell * 0.5;
}
...
function setWeapon(ent, typeIdx, options) {
  ...
  if(typeof resetTapeswordCombat==='function') resetTapeswordCombat(ent);
  if (!ent) return;
  ...
  // Магический посох
  ent._wandBarrierActive=false;
  ent._wandGesture=null;
  ...
  if(def.category === 'tapesword'){
    const base=(typeof PROJECT_PATH_AUDIO!=='undefined')?PROJECT_PATH_AUDIO:'';
    ent._weaponUrl=base+'Source/Weapon/Tapesword/T_TapeswordHand.png';
    ...
    ent._weaponCache[cacheKey+'_ring']=ent._tapeswordRingUrl;
    return;
  }
  ...
  if (ent._weaponCache[cacheKey]) {
    ...
    if(def.category==='tapesword'){
      const base=(typeof PROJECT_PATH_AUDIO!=='undefined')?PROJECT_PATH_AUDIO:'';
      ent._tapeswordRingUrl=ent._weaponCache[cacheKey+'_ring']||base+'Source/Weapon/Tapesword/T_TapeswordRing.png';
      ent._tapeswordRingImg=loadSpriteImage(ent._tapeswordRingUrl);
    }
    return;
  }
...
function droppedWeaponPixelLen(ent){
  ...
  if(def.key === 'bow') return bowVisualPixelLen();
  if(def.key === 'crossbow') return SWORD_LEN * 0.4 * sv('swlen');
  if(def.key === 'flail'){
...
function disarmEntity(ent, kickVx, kickVy){
  ...
  const isRangedDrop = defW.key === 'bow' || defW.key === 'crossbow';
  const tableSpeed = defW.throwSpeed || 10;
  const baseSpeed = isRangedDrop
    ? tableSpeed * (0.22 + Math.random() * 0.06)
    : 6 + Math.random() * 4; // 6-10 пикселей/кадр
  ...
  const finalVy = kickVy !== undefined ? kickVy : Math.sin(angle) * baseSpeed + (isRangedDrop ? 0 : -2); // чуть вверх
  ...
  DROPPED_WEAPONS.push({  
    ...
    tapeswordRingImg: ent._tapeswordRingImg,
    tapeswordRingUrl: ent._tapeswordRingUrl,
    len: droppedWeaponPixelLen(ent),
    owner: ent,
    ownerImmuneUntil: GameTime + 0.4,
    velocityDecay: isRangedDrop ? Math.max(0.82, 1 - (Math.hypot(finalVx, finalVy) / ((typeof CELL_PX === 'number' ? CELL_PX : 55) * 3))) : undefined,
...
function throwWeapon(ent){
  ...
  const c = $.POS.body(ent);
  const def = WEAPON_TYPES[ent.weaponType] || WEAPON_TYPES[0];
  const isRangedThrow = def.key === 'bow' || def.key === 'crossbow';
  const spd = isRangedThrow ? (def.throwSpeed || 10) * 0.3 : (def.throwSpeed || 10) / 2;
  ...
  DROPPED_WEAPONS.push({
    ...
    owner: ent,
    ownerImmuneUntil: GameTime + 0.4,
    velocityDecay: isRangedThrow ? Math.max(0.82, 1 - (spd / ((typeof CELL_PX === 'number' ? CELL_PX : 55) * 3))) : undefined,
    _noPickupUntil: GameTime + 0.4,
    rot: rotAngle,
...
// (spinMin/spinMax — из встроенных значений WEAPON_TYPES).
...
function pointSegmentDistance(px, py, ax, ay, bx, by){
  ...
  const t=l2>0 ? $.M.clamp(((px-ax)*dx+(py-ay)*dy)/l2,0,1) : 0;
  const x=ax+dx*t, y=ay+dy*t;
  return {d:Math.hypot(px-x,py-y),x,y,t};
}
function segmentSegmentDistance(ax, ay, bx, by, cx, cy, dx, dy){
  const d1=pointSegmentDistance(ax,ay,cx,cy,dx,dy);
  const d2=pointSegmentDistance(bx,by,cx,cy,dx,dy);
  const d3=pointSegmentDistance(cx,cy,ax,ay,bx,by);
  const d4=pointSegmentDistance(dx,dy,ax,ay,bx,by);
  let best=d1;
  if(d2.d<best.d) best=d2;
  if(d3.d<best.d) best={d:d3.d,x:cx,y:cy,t:d3.t};
  if(d4.d<best.d) best={d:d4.d,x:dx,y:dy,t:d4.t};
  return best;
}function tryDroppedWeaponBodyHit(w, prevX, prevY, flySpd){
  if(!w || GameTime < (w._noDamageUntil||0)) return false;
  const wDefContact = WEAPON_TYPES[w.weaponType] || WEAPON_TYPES[0];
  const minContactSpeed = (w.isThrow || wDefContact.key === 'bow' || wDefContact.key === 'crossbow') ? 0.8 : 2.5;
  if(flySpd <= minContactSpeed) return false;
  const candidates = [P, D, ...ALL_BOTS].filter((ent,i,arr)=>ent && arr.indexOf(ent)===i);
  for(const ent of candidates){
    if(!ent || ent.hp <= 0 || ent._awaitingReveal) continue;
    if(ent === w.owner && GameTime < (w.ownerImmuneUntil||0)) continue;
    if(typeof FactionRules !== 'undefined' && w.owner && !FactionRules.canDamage(w.owner, ent)) continue;
    const c = $.POS.body(ent);
    const hitR = 24 * (isBot(ent) ? sv('cscl')*sv('botscale')*(ent._bodyScaleMult||1) : sv('cscl'));
    const pathDX=w.x-prevX, pathDY=w.y-prevY, pathL2=pathDX*pathDX+pathDY*pathDY;
    const pathT=pathL2>0 ? $.M.clamp(((c.x-prevX)*pathDX+(c.y-prevY)*pathDY)/pathL2,0,1) : 1;
    const nearX=prevX+pathDX*pathT, nearY=prevY+pathDY*pathT;
    const d = Math.hypot(c.x - nearX, c.y - nearY);
    if(d >= hitR) continue;
    w.x = nearX; w.y = nearY;
    const wDefHit = wDefContact;
    ...
    applyDamage(ent, dmg, w.owner, { weaponDamageKey: wDefHit.key, isMagic: false, isExplosion: false, knockbackMult: 1.2, hitstopFrames: 3, shakePower: dmg > 15 ? 5 : 3, textColor: '#ff8844', textSuffix: 'HIT', bloodCount: 6, playSound: false });
...
function updateDroppedWeapons(dt){
  ...
  for(let i = DROPPED_WEAPONS.length - 1; i >= 0; i--){
    ...
    if(preSpd > 0.4){
      ...
      if(bounced){
        bounceWeapon(w, 0, 1, 0.6);
        $.FX.hit({x:w.x, y:w.y-8, t:'✦', life:18, big:false, col:'#ccccff'});
        $.S.play('clash');
      }
    }

    if(tryDroppedWeaponBodyHit(w, prevX, prevY, Math.hypot(w.vx, w.vy))) continue;
    ...
    if(flySpdPre > 0.5){
      ...
      for(const ent of candidatesDef){
        ...
        if(typeof wandBarrierContains==='function' && wandBarrierContains(ent,w.x,w.y,12)){
          const g=wandBarrierGeometry(ent),nx=w.x-g.x,ny=w.y-g.y,nl=Math.hypot(nx,ny)||1;
          bounceWeapon(w,nx/nl,ny/nl,0.65);
          deflected=true; hitShield=true;
          if(typeof applyShieldBlockFX==='function') applyShieldBlockFX(w.x,w.y,w.owner,ent,{hitstopMag:0});
        }

        // Клинок соперника
        if(!deflected && ent.hasWeapon !== false){
          ...
          const hit = segmentSegmentDistance(prevX, prevY, w.x, w.y, piv.x, piv.y, tipX, tipY);
          if(hit.d < 9){
            let nx = w.x-hit.x, ny = w.y-hit.y;
            const nl = Math.hypot(nx,ny) || 1;
            nx/=nl; ny/=nl;
            w.x = hit.x + nx*9; w.y = hit.y + ny*9;
    ...
    // Затухание скорости
    const velocityDecay = Math.pow(w.velocityDecay || 0.9887, step);
...
function drawDroppedTapesword(w, rot){
  const handImg = w.img || (w.url ? loadSpriteImage(w.url) : null);
  const ringImg = w.tapeswordRingImg || (w.tapeswordRingUrl ? loadSpriteImage(w.tapeswordRingUrl) : null);
  const L = w.len || SWORD_LEN;
  const links = typeof TAPESWORD_BASE_LINKS !== 'undefined' ? TAPESWORD_BASE_LINKS : 9;
  const segmentScale = typeof TAPESWORD_SEGMENT_DRAW_SCALE !== 'undefined' ? TAPESWORD_SEGMENT_DRAW_SCALE : 1.2;
  const lastSegmentScale = typeof TAPESWORD_LAST_SEGMENT_DRAW_SCALE !== 'undefined' ? TAPESWORD_LAST_SEGMENT_DRAW_SCALE : 1.36;
  const overlap = typeof TAPESWORD_SEGMENT_OVERLAP !== 'undefined' ? TAPESWORD_SEGMENT_OVERLAP : 0.58;
  const segmentH = (L / links) * segmentScale;
  ...
  if(handImg && handImg.complete && handImg.naturalWidth > 0){
    const h = L * 0.34;
    const width = h * spriteAspectFor(handImg);
    ctx.drawImage(handImg, -width/2, -h*0.12, width, h);
  }

  let y = -segmentH * 0.15;
  for(let i = 0; i < links; i++){
    const h = (L / links) * (i === links - 1 ? lastSegmentScale : segmentScale);
    y -= step;
    if(ringImg && ringImg.complete && ringImg.naturalWidth > 0){
      const width = h * spriteAspectFor(ringImg);
      ctx.drawImage(ringImg, -width/2, y - h/2, width, h);
    } else {
...
function drawDroppedWeapons(){
  for(const w of DROPPED_WEAPONS){
    ...
    if(wDef.key === 'tapesword'){
      drawDroppedTapesword(w, rot);
      continue;
    }
    ...
    else if(weaponKey === 'bow' || weaponKey === 'crossbow'){
...
function isRangedWeapon(ent){
  return $.IS(ent, 'crossbow', 'bow', 'magicstaff');
}
...
function spendDodgeStamina(ent,baseCost){
  ...
  const key=weaponKeyOf(ent);
  const cost=baseCost*((key==='bow' && rested) || key==='wand' ? 0.5 : 1);
  ent._lastDodgeAt=GameTime;
