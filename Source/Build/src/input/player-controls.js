...
(function(){
  ...
  function dodge(entity, moveX, moveY, charge){
    ...
    const force=(cell*1.5/26)*(1+charge*15/8);
    if(typeof clearPostDodgeSlow==='function') clearPostDodgeSlow(entity);
    ...
    spendDodgeStamina(entity,30);
    if(typeof applyPostDodgeSlow==='function') applyPostDodgeSlow(entity);
  ...
  function updateManualEntity(entity, pad, dt){
    ...
    if(justPressed(pad,12) && typeof window.toggleSwordStyleForEntity==='function') window.toggleSwordStyleForEntity(entity);
    entity._manualControl=true;
    entity._playerSlot=gamepadSlot;
  }

  function clearAutoSwordStyles(){
    const entities=[typeof P!=='undefined'?P:null].concat(typeof ALL_BOTS==='undefined'?[]:ALL_BOTS);
    for(const entity of entities){
      if(!entity) continue;
      entity._autoSwordStyle=null;
      entity._autoSwordStyleIdx=null;
    }
  }

  function updateAutoSwordStyles(){
    if(typeof cb!=='function' || !cb('autostance')){
      clearAutoSwordStyles();
      return;
    }
    syncSlots();
    const hysteresis=10*Math.PI/180;
    for(const slot of slots){
      const entity=slot.source ? slot.entity : null;
      if(!entity) continue;
      const melee=entity.hasWeapon!==false && !isRangedWeapon(entity) && weaponKeyOf(entity)!=='flail';
      if(!melee){ entity._autoSwordStyle=null; entity._autoSwordStyleIdx=null; continue; }
      const enemy=typeof FactionRules!=='undefined' ? FactionRules.nearestEnemy(entity) : null;
      let nextIdx=0;
      if(enemy){
        const ec=$.POS.body(entity), tc=$.POS.body(enemy);
        const axisAngle=Math.atan2(Math.abs(tc.y-ec.y),Math.abs(tc.x-ec.x));
        const previous=entity._autoSwordStyleIdx;
        if(previous===0) nextIdx=axisAngle<Math.PI/4-hysteresis?1:0;
        else if(previous===1) nextIdx=axisAngle>Math.PI/4+hysteresis?0:1;
        else nextIdx=axisAngle>Math.PI/4?0:1;
      }
      entity._autoSwordStyleIdx=nextIdx;
      entity._autoSwordStyle=SWORD_STYLES[nextIdx];
      if(entity!==P && entity._aiState) entity._aiState._styleVals=SWORD_STYLES[nextIdx];
    }
  }
  ...
  function update(dt){
    ...
    if(gamepadSlot > 0){
      ...
      if(pad) updateManualEntity(entity,pad,dt);
        ...
        keys.w=keys.a=keys.s=keys.d=false; entity._aiState._fakeMDown=false; entity._manualAttackInput=false; entity._shieldHeld=false; entity._shieldDashCharging=false;
      }
    }
  ...
  function afterEntityUpdate(entity,dt){
    ...
    if(weaponKeyOf(entity)==='flail') flailInput(entity,!!entity._manualAttackInput,gestureAngle);
    else entity._flailPress=false;
  ...
  window.LocalPlayerControls={
    ...
    updateAutoSwordStyles,clearAutoSwordStyles,
